...
/**
 * Created by chensicong on 10/16/17.
 */

// command + N can auto implements(override) methods
// # views in Comment == #comments + 1
// private Context context; context is father class with resources
// getActivity() : get context with UI

// in relational db, comment is a same level table with event, while in NoSQL, we can set comment as sub table of Event



public class CommentAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder>  {

    // variable - constructor - class - override - helper function

    private Context context;
    private final static int TYPE_EVENT = 0;
    private final static int TYPE_COMMENT = 1;      // enum: suitable for typo (String compare, wrap string)
    ...
    public void setCommentNumber(int commentNumber) {
        this.event.setCommentNumber(commentNumber);
    }
    ...
    // claim a ViewHolder
    // why EventViewHolder here:
    // create Event detail in comment View
    ...
    public class CommentViewHolder extends RecyclerView.ViewHolder {
        ...
        public View layout;                     // we will use this layout later, when define onClick...
                                                // pass in view from father class
    ...
    // create a ViewHolder, depent on the viewType (0,1)
    // initialization only, override will not run again
    @Override
    ...
    // return holder, passed to onBindViewHolder as final RecyclerView.ViewHolder holder
    ...
    // can also modify event in the event detail view (after we click the event list, to go into event list)
    ...
    private void configureEventView(final EventViewHolder holder) {
        ...
        // load event image (see if this event contains image)
        if (event.getImgUri() != null) {
            final String url = event.getImgUri();
            holder.eventImgView.setVisibility(View.VISIBLE);

            // load image : put it background thread
            // getBitmapFromURL : depend on internet connection
            // put UI related work to UI thread
            new AsyncTask<Void, Void, Bitmap>(){
                ...
                // return Utils.getBitmapFromURL to next method(onPostExecute) as bitmap
        ...
        // can like the current event in event detail view
    ...
    private void configureCommentView(final CommentViewHolder commentHolder, final int position) {
        ...
        //Why is position - 1?
        // #comments: n
        // position: event detail + n comments -> n+!
        // so we should let position - 1 when "configure" (draw)
        final Comment comment = commentList.get(position - 1);      // claim comment from commentList
